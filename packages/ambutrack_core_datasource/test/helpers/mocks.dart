...
class MockUsersDataSource implements UsersDataSource {
  ...
  final StreamController<List<UserEntity>> _allUsersController =
  ...
  Future<List<UserEntity>> getUsersByDateRange(
    ...
    return _users.values.where((user) {
      return user.createdAt.isAfter(startDate) &&
          user.createdAt.isBefore(endDate);
    }).toList();
  }
  ...
  Stream<List<UserEntity>> watchSearchResults(
    ...
    return watchAll().asyncMap(
        (_) => searchUsers(query, limit: limit, onlyActive: onlyActive));
  }
...
/// Mock Dio response helpers
class MockDioHelpers {
  static Response<T> createResponse<T>(
    ...
    return Response<T>(
      ...
      headers: Headers.fromMap(headers?.map((key, value) => MapEntry(key, [value.toString()])) ?? {}),
...
/// Firestore mock helpers (simplified without mockito)
class MockFirestoreHelpers {
  /// Creates sample document data for testing
  static Map<String, dynamic> createDocumentData(
    String id,
  ...
  /// Creates sample query result for testing
  static Map<String, dynamic> createQueryResult(
