...
CREATE TABLE IF NOT EXISTS stock_vehiculo (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  vehiculo_id UUID REFERENCES tvehiculos(id) ON DELETE CASCADE,
  producto_id UUID REFERENCES productos(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS movimientos_stock (
  ...
  vehiculo_destino_id UUID REFERENCES tvehiculos(id),
...
JOIN tvehiculos v ON sv.vehiculo_id = v.id
JOIN productos p ON sv.producto_id = p.id
JOIN categorias_equipamiento c ON p.categoria_id = c.id
LEFT JOIN stock_minimo_por_tipo sm ON sm.producto_id = p.id AND sm.tipo_vehiculo = v.tipo_vehiculo;
...
BEGIN
  ...
  JOIN tvehiculos v ON sv.vehiculo_id = v.id
  JOIN productos p ON sv.producto_id = p.id
  JOIN stock_minimo_por_tipo sm ON sm.producto_id = p.id AND sm.tipo_vehiculo = v.tipo_vehiculo
