...
/// Diálogo para seleccionar un rango de días a eliminar de una ausencia
class EliminarDiasParcialDialog extends StatefulWidget {
  const EliminarDiasParcialDialog({
  ...
  @override
  State<EliminarDiasParcialDialog> createState() =>
      _EliminarDiasParcialDialogState();
}

class _EliminarDiasParcialDialogState extends State<EliminarDiasParcialDialog> {
  ...
  void initState() {
    ...
    _fechaInicioEliminar = widget.ausencia.fechaInicio;
    _fechaFinEliminar = widget.ausencia.fechaFin;
  }
  ...
  int get _diasRestantes {
    return widget.ausencia.diasAusencia - _diasAEliminar;
  }

  /// Calcula los periodos que quedarán después de eliminar
  List<_PeriodoRestante> get _periodosRestantes {
    final DateTime inicioOrig = widget.ausencia.fechaInicio;
    final DateTime finOrig = widget.ausencia.fechaFin;
    ...
    if (_fechaInicioEliminar.isAfter(inicioOrig)) {
      final DateTime finAntes = _fechaInicioEliminar.subtract(const Duration(days: 1));
    ...
    if (_fechaFinEliminar.isBefore(finOrig)) {
      final DateTime inicioDespues = _fechaFinEliminar.add(const Duration(days: 1));
    ...
    return AppDialog(
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: <Widget>[
            // Info de la ausencia original
            _buildInfoAusenciaOriginal(),
  ...
  Widget _buildInfoAusenciaOriginal() {
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          _buildInfoRow(
            ...
            'Días totales',
            '${widget.ausencia.diasAusencia} días',
          ),
        ],
      ),
    );
        ...
        children: <Widget>[
          Row(
            children: <Widget>[
              ...
              const SizedBox(width: AppSizes.spacingSmall),
              Text(
                eliminaTodo ? 'Se eliminará toda la ausencia' : 'Resultado después de eliminar',
                style: GoogleFonts.inter(
                  fontSize: 14,
                  fontWeight: FontWeight.w600,
                  color: eliminaTodo ? AppColors.warning : AppColors.success,
          ...
          if (!eliminaTodo) ...<Widget>[
            ...
            const SizedBox(height: AppSizes.spacingSmall),
            Text(
              'Esta acción eliminará completamente la ausencia del registro.',
              style: GoogleFonts.inter(
                fontSize: 13,
  ...
  Future<void> _selectDate({required bool isStart}) async {
    final DateTime? picked = await showDatePicker(
      ...
      firstDate: widget.ausencia.fechaInicio,
      lastDate: widget.ausencia.fechaFin,
...
Future<({DateTime fechaInicio, DateTime fechaFin})?> showEliminarDiasParcialDialog({
  required BuildContext context,
  required AusenciaEntity ausencia,
  required String nombrePersonal,
}) async {
  final Object? result = await showDialog<Object>(
    context: context,
    barrierDismissible: false,
    builder: (BuildContext context) => EliminarDiasParcialDialog(
      ausencia: ausencia,
