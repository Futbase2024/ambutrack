...
/// Tabla de gestión de Categorías de Vehículo
class CategoriaVehiculoTable extends StatefulWidget {
  const CategoriaVehiculoTable({
    super.key,
  ...
  @override
  State<CategoriaVehiculoTable> createState() => _CategoriaVehiculoTableState();
}

class _CategoriaVehiculoTableState extends State<CategoriaVehiculoTable> {
  ...
  @override
  void didUpdateWidget(CategoriaVehiculoTable oldWidget) {
  ...
  Widget build(BuildContext context) {
    return BlocListener<CategoriaVehiculoBloc, CategoriaVehiculoState>(
      listener: (BuildContext context, Object? state) async {
        if (_isDeleting && _loadingDialogContext != null) {
          if (state is CategoriaVehiculoLoaded || state is CategoriaVehiculoError) {
            ...
            } else if (state is CategoriaVehiculoLoaded) {
      ...
      child: BlocBuilder<CategoriaVehiculoBloc, CategoriaVehiculoState>(
        builder: (BuildContext context, Object? state) {
          if (state is CategoriaVehiculoLoaded) {
            List<CategoriaVehiculoEntity> filtrados = _filterTipos(state.categorias);
            ...
            final List<CategoriaVehiculoEntity> tiposPaginados = totalItems > 0
                ? filtrados.sublist(startIndex, endIndex)
                : <CategoriaVehiculoEntity>[];
            ...
            return Column(
              ...
              children: <Widget>[
                if (state.categorias.length != filtrados.length && filtrados.isNotEmpty)
                  Padding(
                    padding: const EdgeInsets.only(bottom: AppSizes.spacing),
                    child: Text(
                      'Mostrando ${filtrados.length} de ${state.categorias.length} categorías',
                ...
                Expanded(
                  child: AppDataGridV5<CategoriaVehiculoEntity>(
                    ...
                    rows: tiposPaginados,
                    buildCells: (CategoriaVehiculoEntity tipo) => <DataGridCell>[
                    ...
                    emptyMessage: hasFilters
                        ...
                        : 'No hay categorías de vehículo registrados',
                    onEdit: (CategoriaVehiculoEntity tipo) => _editTipo(context, tipo),
                    onDelete: (CategoriaVehiculoEntity tipo) => _confirmDelete(context, tipo),
                  ),
                ),
  ...
  List<CategoriaVehiculoEntity> _filterTipos(List<CategoriaVehiculoEntity> tipos) {
    if (widget.searchQuery.isEmpty) {
      return tipos;
    }

    final String query = widget.searchQuery.toLowerCase();
    return tipos.where((CategoriaVehiculoEntity tipo) {
      return tipo.nombre.toLowerCase().contains(query) ||
          (tipo.descripcion?.toLowerCase().contains(query) ?? false);
    }).toList();
  }

  List<CategoriaVehiculoEntity> _sortTipos(List<CategoriaVehiculoEntity> tipos) {
    if (_sortColumnIndex == null) {
      return tipos;
    }

    final List<CategoriaVehiculoEntity> sorted = List<CategoriaVehiculoEntity>.from(tipos)
      ..sort((CategoriaVehiculoEntity a, CategoriaVehiculoEntity b) {
        int comparison = 0;
  ...
  Future<void> _editTipo(BuildContext context, CategoriaVehiculoEntity tipo) async {
    await showDialog<void>(
      context: context,
      builder: (BuildContext dialogContext) => BlocProvider<CategoriaVehiculoBloc>.value(
        value: context.read<CategoriaVehiculoBloc>(),
        child: CategoriaVehiculoFormDialog(categoriaVehiculo: tipo),
      ),
    );
  }

  Future<void> _confirmDelete(BuildContext context, CategoriaVehiculoEntity tipo) async {
    final bool? confirmed = await showConfirmationDialog(
      context: context,
      title: 'Confirmar Eliminación',
      message: '¿Estás seguro de que deseas eliminar este categoría de vehículo? Esta acción no se puede deshacer.',
    ...
    if (confirmed == true && context.mounted) {
        ...
        showDialog<void>(
          ...
          builder: (BuildContext dialogContext) {
            ...
            return const AppLoadingOverlay(
              message: 'Eliminando categoría de vehículo...',
      ...
      if (context.mounted) {
        context.read<CategoriaVehiculoBloc>().add(CategoriaVehiculoDeleteRequested(tipo.id));
      }
    }
  }
  ...
  Widget _buildNombreCell(CategoriaVehiculoEntity tipo) {
    return Text(
      tipo.nombre,
      style: GoogleFonts.inter(
        fontSize: 14,
  ...
  Widget _buildDescripcionCell(CategoriaVehiculoEntity tipo) {
    return Text(
  ...
  Widget _buildEstadoCell(CategoriaVehiculoEntity tipo) {
    return Container(
...
class CategoriaVehiculoSearchField extends StatefulWidget {
  const CategoriaVehiculoSearchField({
    super.key,
    required this.searchQuery,
  ...
  @override
  State<CategoriaVehiculoSearchField> createState() => _CategoriaVehiculoSearchFieldState();
}

class _CategoriaVehiculoSearchFieldState extends State<CategoriaVehiculoSearchField> {
  ...
  @override
  void didUpdateWidget(CategoriaVehiculoSearchField oldWidget) {
    super.didUpdateWidget(oldWidget);
