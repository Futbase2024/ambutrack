...
class ModernDataTable<T> extends StatefulWidget {
  ...
  final List<ModernDataRow<T>> rows;
        ...
        child: Row(
          children: <Widget>[
            ...List<Widget>.generate(widget.columns.length, (int index) {
              final ModernDataColumn column = widget.columns[index];
  ...
  Widget _buildBody() {
    return ListView.separated(
      ...
      itemBuilder: (BuildContext context, int index) {
        final ModernDataRow<T> row = widget.rows[index];
              ...
              decoration: BoxDecoration(
                color: isHovered
                    ...
                    : widget.alternateRowColor && !isEven
                        ...
                        : Colors.white,
                borderRadius: index == widget.rows.length - 1
...
/// Columna del DataTable moderno
class ModernDataColumn {
  const ModernDataColumn({
    required this.label,
    this.flex = 1,
    this.sortable = false,
  });
...
/// Fila del DataTable moderno
class ModernDataRow<T> {
  const ModernDataRow({
    required this.data,
    required this.cells,
  });
